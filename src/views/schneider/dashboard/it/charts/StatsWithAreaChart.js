...
const StatsWithAreaChart = props => {
  ...
  return (
    <Card {...rest}>
      ...
      >
        {icon ? <Avatar className='avatar-stats p-50 m-0' color={`light-${color}`} icon={icon} /> : ''}
        <h2 className='fw-bolder mt-1'>{stats}</h2>
        <p className='card-text'>{statTitle}</p>
      </CardBody>
      <Chart options={options} series={series} type={type} height={height ? height : 100} color={color} />
    </Card>
  )
}
